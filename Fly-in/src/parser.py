...
class Parser:
    ...
    def _parse_line(self, n_line: int, line: str) -> None:
        ...
        if key == "nb_drones":
            if self.data_parsed["nb_drones"] is not None:
                raise ParserError("Multiple nb_drones definitions")
    ...
    def _parse_zone(self, key: str, value: str) -> None:
        ...
        info = value.split()
        if len(info) < 3:
            raise ParserError("Zone definition must have "
                              "at least name and coordinates")
        zone_name = info[0]
        if '-' in zone_name or ' ' in zone_name:
            raise ParserError(f"Zone name '{zone_name}' "
        ...
        if len(info) > 3:
            ...
            for data in metadata:
                ...
                if data_key == "zone":
                    if data_value.upper() not in ZoneType.__members__:
                        raise ParserError(f"Invalid zone type: {data_value}")
                    zone_info["zone_type"] = ZoneType[data_value.upper()]
                elif data_key == "color":
                    if ' ' in data_value:
                        raise ParserError(
                            f"Color '{data_value}' must be a single word")
        ...
        if key == 'start_hub':
            ...
            if self.data_parsed["end_hub"] is not None:
                raise ParserError("Multiple end_hub definitions found")
    ...
    def _parse_connection(self, value: str) -> None:
        ...
        info = value.split()
        if not info:
            raise ParserError("Emptry connection line")
        if '-' not in info[0]:
            raise ParserError("Connection must be in the format 'zone1-zone2'")
        zone1, zone2 = info[0].split('-', 1)
        if not zone1 or not zone2:
            raise ParserError("Connection must specify two zones")
        if zone1 == zone2:
            raise ParserError("Connection cannot be between the same zone")
        if (zone1 not in self.data_parsed["zones"]
           or zone2 not in self.data_parsed["zones"]):
            raise ParserError("Connection references unknown"
                              f" zone: {zone1} or {zone2}")
        sorted_zones = sorted([zone1, zone2])
        edge: Tuple[str, str] = (sorted_zones[0], sorted_zones[1])
        if any(conn.key == edge for conn in self.data_parsed["connections"]):
            raise ParserError(f"Duplicate connection between {zone1}-{zone2}")
        ...
        if len(info) > 1:
            ...
            if data_key == 'max_link_capacity':
                ...
                connection_info["max_link_capacity"] = int(data_value)
            else:
                raise ParserError(f"Unknown metadata key: {data_key}")
    ...
    def _data_checker(self) -> None:
        if self.data_parsed["start_hub"] is None:
            raise ParserError("Start hub (start_hub) is missing.")
        if self.data_parsed["end_hub"] is None:
            raise ParserError("End hub (end_hub) is missing.")
        if self.data_parsed["start_hub"].zone_type == ZoneType.BLOCKED:
            raise ParserError("Start hub cannot be of type BLOCKED")
        if self.data_parsed["end_hub"].zone_type == ZoneType.BLOCKED:
            raise ParserError("End hub cannot be of type BLOCKED")
        if self.data_parsed["nb_drones"] is None:
            raise ParserError("Number of drones (nb_drones) is missing.")
